...
def conversion(
    character_name,
    cascade_file=os.path.join(os.getcwd(), "data-models", "lbpcascade_animeface.xml"),
):
    ...
    for raw_image in raw_images:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite(
                os.path.join("dataset", "train", character_name, filename + ".jpg"),
                crop_image,
            )
...
def resize(character_name):
    ...
    for file in files:
        ...
        cv2.imwrite(
            os.path.join("dataset", "train", character_name, file), resized_image
        )
...
def main():
    if len(sys.argv) != 2:
        sys.stderr.write(
            "Usage: python preparation.py <character_name> (character name must be a folder inside the 'raw' and 'train' folder, and must not contain spaces!) \n"
        )
        sys.exit(-1)
