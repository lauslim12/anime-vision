...
def create_custom_classifier(visual_recognition):
    with open("maki.zip", "rb") as maki, open("riko.zip", "rb") as riko, open(
        "hifumi.zip", "rb"
    ) as hifumi:
        ...
        model = visual_recognition.create_classifier(
            ...
            positive_examples={
                "nishikino_maki": maki,
                "sakurauchi_riko": riko,
                "takimoto_hifumi": hifumi,
            },
        ).get_result()
...
def get_classifiers(visual_recognition):
    ...
    if not classifiers["classifiers"]:
        sys.exit(
            "Your IBM Cloud Account has no classifiers! Please ensure that you have one!"
        )
    else:
...
def test_classifier(visual_recognition, classifier_id):
    if not sys.argv[2]:
        sys.exit("You must define an image to be checked in the IBM Cloud!")

    with open(sys.argv[2], "rb") as image:
        classes = visual_recognition.classify(
            images_file=image, threshold="0.6", classifier_ids=classifier_id
        ).get_result()
...
def main():
    ...
    if len(sys.argv) < 2:
        sys.stderr.write(
            "Usage: python ibm-cloud.py <action> (check, delete, create, test) <image_path> \n"
        )
        sys.exit(-1)
